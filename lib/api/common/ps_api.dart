...
abstract class PsApi {
  ...
  Future<List<dynamic>> getList(String url) async {
    ...
    try {
      final String fullUrl = '${PsConfig.ps_app_url}$url';
      print('DEBUG: getList - Full URL: $fullUrl');
      print('DEBUG: getList - PsConfig.ps_app_url: ${PsConfig.ps_app_url}');
      print('DEBUG: getList - Input url: $url');
      
      final Response response = await client.get(Uri.parse(fullUrl));
  ...
  Future<PsResource<R>> getServerCall<T extends PsObject<dynamic>, R>(
    ...
    try {
      final String fullUrl = '${PsConfig.ps_app_url}$url';
      print('DEBUG: getServerCall - Full URL: $fullUrl');
      print('DEBUG: getServerCall - PsConfig.ps_app_url: ${PsConfig.ps_app_url}');
      print('DEBUG: getServerCall - Input url: $url');
      ...
      if (psApiResponse.isSuccessful()) {
        ...
        // PATCH: Extract 'data' field if present
        dynamic dataToParse = hashMap;
        if (dataToParse is Map && dataToParse.containsKey('data')) {
        ...
        if (dataToParse is! Map) {
          ...
          dataToParse.forEach((dynamic data) {
            tList.add(obj.fromMap(data as dynamic));
          });
          return PsResource<R>(PsStatus.SUCCESS, '', tList as R? ?? R as R?);
        } else {
          // Handle single object response - wrap in list if R is List
          if (R.toString().contains('List')) {
            final List<T> tList = <T>[obj.fromMap(dataToParse)];
            return PsResource<R>(PsStatus.SUCCESS, '', tList as R? ?? R as R?);
          } else {
            return PsResource<R>(PsStatus.SUCCESS, '', obj.fromMap(dataToParse));
          }
        }
      } else {
  ...
  Future<PsResource<R>> postData<T extends PsObject<dynamic>, R>(
    ...
    try {
      final String fullUrl = '${PsConfig.ps_app_url}$url';
      print('DEBUG: postData - Full URL: $fullUrl');
      print('DEBUG: postData - PsConfig.ps_app_url: ${PsConfig.ps_app_url}');
      print('DEBUG: postData - Input url: $url');
      ...
      final Response response = await client
        ...
        print(e.message);
        return Response('', 500); // Return an empty response with status code 500
      });
  ...
  Future<PsResource<R>> postUploadImage<T extends PsObject<dynamic>, R>(T obj,
    ...
    try {
      final String fullUrl = '${PsConfig.ps_app_url}$url';
      print('DEBUG: postUploadImage - Full URL: $fullUrl');
      print('DEBUG: postUploadImage - PsConfig.ps_app_url: ${PsConfig.ps_app_url}');
      print('DEBUG: postUploadImage - Input url: $url');
