...
class Checkout3View extends StatefulWidget {
  const Checkout3View(
    ...
    this.isWeeklyScheduleOrder, {super.key}
  );
  ...
  _Checkout3ViewState createState() {
    final _Checkout3ViewState state = _Checkout3ViewState();
    updateCheckout3ViewState(state);
    return state;
  }
}
...
class _Checkout3ViewState extends State<Checkout3View> {
  ...
  dynamic payFlutterWave(
    ...
    final Flutterwave flutterwave = Flutterwave(
      ...
      txRef: '${DateFormat('dd-MM-yyyy hh:mm:ss').format(DateTime.now())}-${valueHolder!.loginUserId}',
      amount: amount,
      customer: customer,
  ...
  dynamic payNow(
    ...
    Utils.psPrintDebug(
        basketProvider.checkoutCalculationHelper.totalPrice.toString());
    Utils.psPrintDebug(shopInfoProvider.shopInfo.data!.currencyShortForm!);
    Utils.psPrintDebug(widget.basketList.length);

    Utils.psPrintDebug(valueHolder!.standardShippingEnable == PsConst.ONE);

    Utils.psPrintDebug(valueHolder!.zoneShippingEnable == PsConst.ONE);
    if (psValueHolder.standardShippingEnable == PsConst.ONE) {
      basketProvider.checkoutCalculationHelper.calculate(
          basketList: widget.basketList,
          couponDiscountString: couponDiscountProvider.couponDiscount,
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            SizedBox(width: 50, height: 50, child: const Icon(Icons.payment)),
            Container(
              margin: const EdgeInsets.only(
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            SizedBox(
                width: 50,
                height: 50,
                child: Icon(FontAwesome.paypal, color: PsColors.white)),
            Container(
              margin: const EdgeInsets.only(
                left: PsDimens.space16,
                right: PsDimens.space16,
