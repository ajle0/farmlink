...
late  CategoryDao _categoryDao;
  ...
  Future<dynamic> getCategoryList(
    ...
    if (isConnectedToInternet) {
      final PsResource<List<Category>> resource =
      ...
      if (resource.status == PsStatus.SUCCESS) {
        ...
        int i = 0;
        for (Category data in resource.data!) {
        ...
        // Insert Category
        await _categoryDao.insertAll(primaryKey, resource.data!);
      } else {
  ...
  Future<dynamic> getCategoryListByKey(
      StreamController<PsResource<List<Category>>> categoryListStream,
      bool isConnectedToInternet,
      int limit,
      int offset,
    ...
    if (isConnectedToInternet) {
      final PsResource<List<Category>> resource = await _psApiService
          .getCategoryListByKey(limit, offset, holder.toMap());
