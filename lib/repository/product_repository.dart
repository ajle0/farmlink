...
late  ProductDao _productDao;
  ...
  Future<dynamic> getProductList(
    ...
    if (isConnectedToInternet) {
      ...
      if (resource.status == PsStatus.SUCCESS) {
        ...
        int i = 0;
        for (Product data in resource.data!) {
        ...
        // Insert Product
        await _productDao.insertAll(primaryKey, resource.data!);
      } else {
  ...
  Future<PsResource<List<DownloadProduct>>> postDownloadProductList(
      ...
      PsStatus status,
      {bool isLoadFromServer = true}) async {
    final PsResource<List<DownloadProduct>> resource =
  ...
  Future<dynamic> getAllproductListByCollectionId(
    ...
    // Server Call
    if (isConnectedToInternet) {
      final PsResource<List<Product>> resource = await _psApiService
