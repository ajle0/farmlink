...
class UserProvider extends PsProvider {
  ...
  bool hasLatLng(PsValueHolder psValueHolder) {
    final LatLng latlng = getUserLatLng(psValueHolder);
    if (latlng.latitude == 0.0 ||
        latlng.longitude == 0.0) {
      return false;
    } else {
      if (latlng.latitude.toString() == psValueHolder.lat &&
          latlng.longitude.toString() == psValueHolder.lng) {
        return false;
      } else {
        return true;
      }
  ...
  Future<PsResource<User>?> _submitLoginWithAppleId(fb_auth.User? user) async {
    if (user != null) {
      ...
      if (apiStatus.data != null) {
        ...
        await replaceLoginUserId(apiStatus.data!.userId!);
      }
      return apiStatus;
    } else {
      return null;
  ...
  // build a Firebase credential using *only* the ID token
  ...
  Future<FacebookLoginUserHolder?> _getFirebaseUserWithFacebookId(
    ...
    final String fbToken = result.accessToken!.tokenString;

    // still fetch profile manually if you need it
    ...
    final OAuthCredential fbCred =
        FacebookAuthProvider.credential(fbToken);

    try {
      final fb_auth.User user =
          (await _firebaseAuth.signInWithCredential(fbCred)).user!;
  ...
  Future<fb_auth.User?> signInWithEmailAndPassword(
    ...
    try {
      ...
      final fb_auth.User? user0 = await createUserWithEmailAndPassword(
      ...
      if (user == null) {
        try {
          ...
          print(e);
          final fb_auth.User? user2 = await createUserWithEmailAndPassword(
  ...
  Future<PsResource<User>> _submitSignUpWithEmailId(
    ...
    if (apiStatus.data != null) {
      final User user = apiStatus.data!;

      //for change email
      await replaceVerifyUserData(apiStatus.data!.userId!,
          apiStatus.data!.userName!, apiStatus.data!.userEmail!, password);
