...
class PsApiService extends PsApi {
  ...
  Future<PsResource<List<Product>>> getProductList(
    ...
    // Add cat_id to URL if present
    if (paramMap['cat_id'] != null && paramMap['cat_id'].toString().isNotEmpty) {
      url += '/cat_id/${paramMap['cat_id']}';
    }
  ...
  Future<PsResource<SearchResult>> getSearchResult(
      Map<String, dynamic> json, int offset) async {
    final String url =
        '${PsUrl.ps_search_result_url}/api_key/${PsConfig.ps_api_key}/offset/$offset';
  ...
  Future<PsResource<List<BestChoice>>> getBestChoiceList() async {
    const String url =
        '${PsUrl.ps_best_choice_url}/api_key/${PsConfig.ps_api_key}';
