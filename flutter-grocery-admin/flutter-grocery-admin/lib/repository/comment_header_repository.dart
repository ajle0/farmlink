...
class CommentHeaderRepository extends PsRepository {
 ...
 late CommentHeaderDao _commentHeaderDao;
  ...
  Future<dynamic> getAllCommentList(
    ...
    if (isConnectedToInternet) {
      final PsResource<List<CommentHeader>> resource =
      ...
      if (resource.status == PsStatus.SUCCESS) {
        ...
        await _commentHeaderDao.insertAll(primaryKey, resource.data!);
      } else {
        if (resource.errorCode == PsConst.ERROR_CODE_10001) {
          await _commentHeaderDao.deleteWithFinder(finder);
        }
