...
class ScheduleCheckoutStatusView extends StatefulWidget {
  const ScheduleCheckoutStatusView({
    super.key,
  ...
  @override
  _ScheduleCheckoutStatusViewState createState() =>
      _ScheduleCheckoutStatusViewState();
}

class _ScheduleCheckoutStatusViewState
    extends State<ScheduleCheckoutStatusView> {
  ...
  Widget build(BuildContext context) {
    ...
    final Widget contentCopyIconWidget = IconButton(
      ...
      onPressed: () {
        Clipboard.setData(ClipboardData(text: widget.scheduleHeader!.transCode ?? ''));
                    ...
                    SingleChildScrollView(
                          ...
                          child: Column(
                            children: <Widget>[
                                    ...
                                    Expanded(
                                      child: Row(
                                        children: <Widget>[
                                          ...
                                          Expanded(
                                            child: Text(
                                                '${Utils.getString(context, 'transaction_detail__trans_no')} : ${widget.scheduleHeader!.transCode}',
                                                style: Theme.of(context)
                                                    .textTheme
                                                    .titleLarge),
                                          ),
                              ...
                              _TransactionNoTextWidget(
                                transationInfoText:
                                    widget.scheduleHeader!.totalItemCount!,
                                title:
                                    '${Utils.getString(context, 'transaction_detail__total_item_count')} :',
                              ),
                              _TransactionNoTextWidget(
                                transationInfoText:
                                    '${widget.scheduleHeader!.currencySymbol} ${Utils.getPriceFormat(widget.scheduleHeader!.totalItemAmount!, valueHolder!)}',
                                title:
                                    '${Utils.getString(context, 'transaction_detail__total_item_price')} :',
                              ),
                              _TransactionNoTextWidget(
                                transationInfoText:
                                    '${widget.scheduleHeader!.currencySymbol} ${Utils.getPriceFormat(widget.scheduleHeader!.discountAmount!, valueHolder!)}',
                                title:
                                    '${Utils.getString(context, 'transaction_detail__discount')} :',
                              ),
                              _TransactionNoTextWidget(
                                transationInfoText:
                                    '${widget.scheduleHeader!.currencySymbol} ${Utils.getPriceFormat(widget.scheduleHeader!.cuponDiscountAmount!, valueHolder!)}',
                                title:
                              ...
                              _TransactionNoTextWidget(
                                transationInfoText:
                                    '${widget.scheduleHeader!.currencySymbol} ${Utils.getPriceFormat(widget.scheduleHeader!.subTotalAmount!, valueHolder!)}',
                                title:
                                    '${Utils.getString(context, 'transaction_detail__sub_total')} :',
                              ),
                              _TransactionNoTextWidget(
                                transationInfoText:
                                    '${widget.scheduleHeader!.currencySymbol} ${Utils.getPriceFormat(widget.scheduleHeader!.taxAmount!, valueHolder!)}',
                                title:
                                    '${Utils.getString(context, 'transaction_detail__tax')}(${provider.psValueHolder!.overAllTaxLabel} %) :',
                              ),
                              _TransactionNoTextWidget(
                                transationInfoText:
                                    '${widget.scheduleHeader!.currencySymbol} ${Utils.getPriceFormat(widget.scheduleHeader!.shippingAmount!, valueHolder!)}',
                                title:
                                    '${Utils.getString(context, 'transaction_detail__shipping_cost')} :',
                              ),
                              _TransactionNoTextWidget(
                                transationInfoText:
                                    '${widget.scheduleHeader!.currencySymbol} ${Utils.calculateShippingTax(widget.scheduleHeader!.shippingAmount, provider.psValueHolder!.shippingTaxValue, valueHolder!)}',
                                title:
                              ...
                              _TransactionNoTextWidget(
                                transationInfoText:
                                    '${widget.scheduleHeader!.currencySymbol} ${Utils.getPriceFormat(widget.scheduleHeader!.balanceAmount!, valueHolder!)}',
                                title:
                      ...
                      Padding(
                        ...
                        child: PSButtonWidget(
                          ...
                          onPressed: () {
                            Navigator.pushNamed(
                                context, RoutePaths.scheduleTransactionDetail,
                                arguments: widget.scheduleHeader);
                          },
                        ),
                      ),
                      const SizedBox(
