            ...
            onPressed: () async {
              if (codeController.text.isEmpty) {
                ...
                if (await Utils.checkInternetConnectivity()) {
                  ...
                  final PsResource<User> apiStatus = await widget.provider
                      .postUserEmailVerify(emailVerifyParameterHolder.toMap());

                  if (apiStatus.data != null) {
                   await widget.provider.replaceVerifyUserData('', '', '', '');
                   await widget.provider.replaceLoginUserId(apiStatus.data!.userId!);

                    if (widget.onProfileSelected != null) {
                      await widget.provider
                          .replaceVerifyUserData('', '', '', '');
                      await widget.provider
                          .replaceLoginUserId(apiStatus.data!.userId!);
                      await widget.onProfileSelected!(apiStatus.data!.userId);
                    } else {
                      Navigator.pop(context, apiStatus.data);
                    }
                  } else {
                    showDialog<dynamic>(
                        context: context,
        ...
        MaterialButton(
          ...
          onPressed: () async {
            if (await Utils.checkInternetConnectivity()) {
              ...
              final PsResource<ApiStatus> apiStatus = await widget.provider
