...
class PS_Model extends CI_Model {
  ...
  */
	...
	*/
	...
	function get_one1( $id,$shop_id = 0 ) {
	...
	function get_all_by( $conds = array(), $limit = false, $offset = false ) {
		...
		$query = $this->db->get();
		error_log('DEBUG SQL: ' . $this->db->last_query());
		print_r('DEBUG SQL: ' . $this->db->last_query());
		return $query;
		 //print_r($this->db->last_query());die;
	}
...
if ($conds['search_term'] == "" && $mindate != "" && $maxdate != "") {
	...
	*/
	...
	Returns recent product
	*/
	function get_all_product_best_choice( $conds = array() )
	{
	
		$sql = "SELECT *  FROM rt_products ORDER BY CASE WHEN id in (". $conds['prd_ids_from_coll'] .") then -1 else id end,id, is_discount asc";
		$query = $this->db->query($sql);
	...
	*/
	...
	function get_all_order_delivery( $conds = array() ){
		...
		if ($conds['status'] == "pending") {
			if(isset($conds['trans_status_id'])) {
				...
				if ($conds['trans_status_id'] != "" || $conds['trans_status_id'] != 0) {
						...
						// $names = array('trans_sts47fe98346e0f80d844d307981eaef7ec', 'trans_sts1432c4708d810e38dc04f017c0b329dc', 'trans_sts29a4b0cd2fa6ae0449e47e9568320f3a');
						$this->db->where_not_in($this->table_name . '.trans_status_id', $conds['trans_status_id']);
						// $this->db->where( $this->table_name . '.trans_status_id!=', $conds['trans_status_id'] );
				}
		...
		// trans status optional condition
		if(isset($conds['is_optional'])){
			$this->db->join('rt_transactions_status', 'rt_transactions_status.id=' . $this->table_name . '.trans_status_id');
	...
	*/
	...
	Returns purchased products count
	*/
	function count_purchased_user_by($conds = array()){
		$this->custom_conds();
		//where clause		
		$this->db->select('core_users.*, count(rt_transactions_header.user_id) as t_count');    
  		$this->db->from('core_users');
  		$this->db->join('rt_transactions_header', 'core_users.user_id = rt_transactions_header.added_user_id');
  		//$this->db->where('core_users.status',1);
  		$this->db->where('rt_transactions_header.shop_id',$conds['shop_id']);
  		...
  		if ( isset( $conds['search_term'] ) || isset( $conds['date'] ) ) {
			...
			if ($conds['search_term'] == "" && $mindate != "" && $maxdate != "") {
				...
				if ($mindate == $maxdate ) {
					...
					$this->db->where("rt_transactions_header.added_date BETWEEN DATE('".$mindate."' - INTERVAL 1 DAY) AND DATE('". $maxdate."' + INTERVAL 1 DAY)");

				} else {
					$this->db->where( 'rt_transactions_header.added_date >=', $mindate );
   					$this->db->where( 'rt_transactions_header.added_date <=', $maxdate );

				}
				$this->db->like( '(user_name', $conds['search_term'] );
				$this->db->or_like( 'user_name)', $conds['search_term'] );
			} else if ($conds['search_term'] != "" && $mindate != "" && $maxdate != "") {
				//got name and 2dates
				if ($mindate == $maxdate ) {
  		...
  		$this->db->group_by('rt_transactions_header.user_id');
  		$this->db->order_by('t_count', "DESC");
	...
	* Gets purchased products record
	*/
	function get_purchased_user_by ( $conds = array(), $limit = false, $offset = false ){

		//$this->custom_conds();
		//where clause
		$this->db->select('core_users.*, count(rt_transactions_header.added_user_id) as t_count');    
  		$this->db->from('core_users');
  		$this->db->join('rt_transactions_header', 'core_users.user_id = rt_transactions_header.added_user_id');
  		//$this->db->where('rt_products.status',1);
  		$this->db->where('rt_transactions_header.shop_id',$conds['shop_id']);
  		...
  		if ( isset( $conds['search_term'] ) || isset( $conds['date'] ) ) {
			...
			if ($conds['search_term'] == "" && $mindate != "" && $maxdate != "") {
				...
				if ($mindate == $maxdate ) {
					...
					$this->db->where("rt_transactions_header.added_date BETWEEN DATE('".$mindate."') AND DATE('". $maxdate."' + INTERVAL 1 DAY)");
					...
					$this->db->where( 'date(rt_transactions_header.added_date) >=', $mindate );
   					$this->db->where( 'date(rt_transactions_header.added_date) <=', $maxdate );
  		...
  		$this->db->group_by('rt_transactions_header.added_user_id');
  		$this->db->order_by('t_count', "DESC");
  		$this->db->order_by('rt_transactions_header.added_date',"desc");
	...
	/**
	  ...
	  * @return     <type>   All by.
	*/
	function get_all_not_in_schedule_orders( $conds = array(), $limit = false, $offset = false ) {

		// where clause
		$this->db->where_not_in('route_id', $conds);
	...
	Returns recent product
	*/
	function get_all_schedule_orders( $conds = array() )
	{

		if($conds['schedule_ids_from_other'] != "") {

			if($conds['schedule_ids_from_route'] != "") {

				// Filter for schedule order route edit (schedule_ids_from_other, schedule_ids_from_route)
				$sql = "SELECT *  FROM  rt_user_schedules_header WHERE shop_id = '". $conds['shop_id'] ."' and schedule_day = '". $conds['schedule_day'] ."'  and id NOT IN(". $conds['schedule_ids_from_other'] .")  ORDER BY CASE WHEN id in (". $conds['schedule_ids_from_route'] .") then -1 else id end, added_date asc";

			} else {

				// Filter for schedule order route create (schedule_ids_from_other)
				$sql = "SELECT *  FROM  rt_user_schedules_header WHERE shop_id = '". $conds['shop_id'] ."' and schedule_day = '". $conds['schedule_day'] ."' and id NOT IN(". $conds['schedule_ids_from_other'] .")";

			}

		} else if($conds['schedule_ids_from_other'] == "") {

			if($conds['schedule_ids_from_route'] != "") {

				// Filter for schedule order edit (schedule_ids_from_route)
				$sql = "SELECT *  FROM rt_user_schedules_header WHERE shop_id = '". $conds['shop_id']."' and schedule_day = '". $conds['schedule_day'] ."' ORDER BY CASE WHEN id in (". $conds['schedule_ids_from_route'] .") then -1 else id end, added_date asc";

			} else {
				// Filter for schedule order create
				$sql = "SELECT *  FROM rt_user_schedules_header WHERE shop_id = '". $conds['shop_id'] ."' and schedule_day = '". $conds['schedule_day'] . "'";
			}
	...
	/**
	  ...
	  * @return     <type>   All by.
	  */
	function get_all_in_schedule_order( $conds = array(), $limit = false, $offset = false ) {

	  // where clause
	  $this->db->where_in('route_id', $conds);
	...
	*/
	...
	function get_all_new_order_assign_record($conds, $limit = false, $offset = false){
		
		$this->db->select('rt_transactions_header.*'); 
		$this->db->from('rt_transactions_header');
		$this->db->join('rt_new_order_assign_log', 'rt_new_order_assign_log.trans_header_id = rt_transactions_header.id');

		if(isset($conds['login_user_id'])){
			
			$this->db->group_start();
			$this->db->where( 'rt_transactions_header.delivery_boy_id','0');
			$this->db->or_where( 'rt_transactions_header.delivery_boy_id','');
			$this->db->group_end();

			$this->db->where( 'rt_new_order_assign_log.delivery_boy_id',$conds['login_user_id']);
		}else{
			$this->db->where( 'rt_transactions_header.delivery_boy_id','0');
			$this->db->or_where( 'rt_transactions_header.delivery_boy_id','');
		}
