...
class User extends PS_Model {
	...
	function custom_conds( $conds = array())
	{
		...
		// current_distance condition
		if ( isset( $conds['current_distance'] )) {
			$this->db->order_by( 'current_distance', 'asc' );
		} else {
			$this->db->order_by( 'added_date', 'desc' );
		}

		// user_id not equal condition (for email uniqueness)
		if (isset($conds['user_id !='])) {
	...
	function save_user( &$user_data, $user_id = false ){
		...
		}elseif (!$user_id && $this->exists( array( 'user_email' => $user_data['user_email']))) {
	 ...
	 * @return bool
	 */
	function save_system_user( &$user_data, $permission_data, $user_id = false )
	{
		// start the transaction
		$this->db->trans_start();
	...
	function delete( $user_id )
	{
		...
		// if error in deleting user, rollback
