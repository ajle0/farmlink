 ...
 */
class Schedule_header extends PS_Model {
	...
	{
		parent::__construct( 'rt_user_schedules_header', 'id', 'sch_hdr_' );
	}
	...
	function custom_conds( $conds = array())
	{
		...
		if ( isset( $conds['schedule_day'] )) {
			$this->db->where( 'schedule_day', $conds['schedule_day'] );
		}

		if ( isset( $conds['schedule_time'] )) {
			$this->db->where( 'schedule_time', $conds['schedule_time'] );
		}

		
		if ( isset( $conds['sch_header_ids'] )) {
			$this->db->where_in( 'id', $conds['sch_header_ids'] );
		}

		if ( isset( $conds1['sch_header_ids'] )) {
			$this->db->where_in( 'id', $conds1['sch_header_ids'] );
		}
