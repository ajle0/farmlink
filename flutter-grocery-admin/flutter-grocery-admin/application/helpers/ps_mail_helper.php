...
<p>
{$shipping_cost} : {$shipping_amount} {$trans_currency}
</p>
<p>
...
if ( !function_exists( 'send_shop_approval_emails' )) {
	...
	function send_shop_approval_emails( $shop_id, $status, $to_who = "", $subject = "" )
	{
		// get ci instance
		$CI =& get_instance();
...
if ( !function_exists( 'send_refund_order_email' )) {
    ...
    function send_refund_order_email( $transaction_header_id, $amount, $to_who = "", $subject = "" )
    {
        // get ci instance
        $CI =& get_instance();
...
if ( !function_exists( 'send_schedule_order_emails' )) {

	function send_schedule_order_emails( $schedule_header_id, $to_who = "", $subject = "" )
	{
		// get ci instance
		$CI =& get_instance();
		...
		$schedule_header_obj = $CI->Schedule_header->get_one($schedule_header_id);
		$shop_name = $CI->Shop->get_one($schedule_header_obj->shop_id)->name;

		$shop_email = $CI->Shop->get_one($schedule_header_obj->shop_id)->email;

		$trans_currency = $CI->Shop->get_one($schedule_header_obj->shop_id)->currency_symbol;

		$user_email =  $CI->User->get_one($schedule_header_obj->added_user_id)->user_email;

		if ($user_email == "") {
			$user_email = $schedule_header_obj->billing_email;
		}

		$user_name =  $CI->User->get_one($schedule_header_obj->added_user_id)->user_name;

		//bank info 
		$bank_account = $CI->Shop->get_one($schedule_header_obj->shop_id)->bank_account;
		$bank_name = $CI->Shop->get_one($schedule_header_obj->shop_id)->bank_name;
		$bank_code = $CI->Shop->get_one($schedule_header_obj->shop_id)->bank_code;
		$branch_code = $CI->Shop->get_one($schedule_header_obj->shop_id)->branch_code;
		$swift_code = $CI->Shop->get_one($schedule_header_obj->shop_id)->swift_code;
		...
		$schedule_details_obj = $CI->Schedule_detail->get_all_by($conds)->result();

		//For Transaction Detials
		for($i=0;$i<count($schedule_details_obj);$i++) 
		{
				if($schedule_details_obj[$i]->product_attribute_id != "") {
					

					$att_name_info  = explode("#", $schedule_details_obj[$i]->product_attribute_name);
					
					$att_price_info = explode("#", $schedule_details_obj[$i]->product_attribute_price);
					...
					$order_items .= $i + 1 .". " . $schedule_details_obj[$i]->product_name . 
					" (". get_msg('price_label') .   $schedule_details_obj[$i]->original_price  . html_entity_decode($trans_currency) . 
					"," . get_msg('qty_label') ." : " . $schedule_details_obj[$i]->qty . ",". get_msg('unit_label') ." : " . $schedule_details_obj[$i]->product_unit_value .' ' . $schedule_details_obj[$i]->product_unit . ") {". $att_info_str ."}<br>";
					...
					$order_items .= $i + 1 .". " . $schedule_details_obj[$i]->product_name . 
					" (". get_msg('price_label') .   $schedule_details_obj[$i]->original_price  . html_entity_decode($trans_currency) . 
					"," . get_msg('qty_label') ." : " . $schedule_details_obj[$i]->qty . ",". get_msg('unit_label') ." : " . $schedule_details_obj[$i]->product_unit_value .' ' . $schedule_details_obj[$i]->product_unit . ") <br>";
					
				}
				
				$sub_total_amt += $schedule_details_obj[$i]->original_price * $schedule_details_obj[$i]->qty;
		...
		$trans_status = $CI->Transactionstatus->get_one($schedule_header_obj->trans_status_id)->title;



		$total_amt = $total_amount .' ' . html_entity_decode($trans_currency);

		$coupon_discount_amount = $schedule_header_obj->coupon_discount_amount;
		$tax_amount = $schedule_header_obj->tax_amount;
		$shipping_amount = $schedule_header_obj->shipping_amount;
		$shipping_tax_amount = (float)$schedule_header_obj->shipping_amount * (float)$schedule_header_obj->shipping_tax_percent;

		$total_balance_amount = ((float)$schedule_header_obj->sub_total_amount + ((float)$schedule_header_obj->tax_amount + (float)$schedule_header_obj->shipping_amount + ((float)$schedule_header_obj->shipping_amount * (float)$schedule_header_obj->shipping_tax_percent)));   
...
<p>
{$trans_code} : {$schedule_header_obj->trans_code}<br/>
</p>
...
<p>
{$memo_label}: {$schedule_header_obj->memo}
</p>
