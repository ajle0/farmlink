...
if ( ! function_exists( 'global_user_check' )) 
{
	function global_user_check( $user_id )
	{
        ...
        if ($user_data->is_empty_object == 1 && $role_id == '4') {
            ...
            $CI->error_response( get_msg( 'deli_boy_not_existed' ));
        } elseif ($is_ban == '1' && $role_id == '4') {
            $CI->error_response( get_msg( 'user_banned' ));
        } elseif ($is_ban == '1'&& $role_id == '5') {
            $CI->error_response( get_msg( 'deli_boy_banned' ));
        }
 ...
 */
	...
	*/
	...
	if ( ! function_exists( 'send_fcm_v1' ))
	{
		function send_fcm_v1($registration_ids, $data, $platform_names)
		{
			require_once FCPATH . 'vendor/autoload.php';
			$serviceAccountFile = APPPATH . 'config/flutter-multivendor-4d3d5-firebase-adminsdk-fbsvc-8a32049c45.json';
			$projectId = 'flutter-multivendor-4d3d5'; // TODO: Replace with your actual Firebase project ID
			...
			$results = [];
			foreach ($registration_ids as $targetToken) {
				$message = [
					'message' => [
						'token' => $targetToken,
						'notification' => [
	...
	if ( ! function_exists( 'send_android_fcm' ))
	{
		function send_android_fcm( $registatoin_ids, $data, $platform_names)
		{
			...
			return send_fcm_v1($registatoin_ids, $data, $platform_names);
		}
	}
