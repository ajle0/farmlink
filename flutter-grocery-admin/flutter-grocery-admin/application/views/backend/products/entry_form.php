<?php if (!isset($selected_cat_id)) { $selected_cat_id = ''; } ?>
<?php if (!isset($product)) { $product = (object) []; } ?>
<?php if (!isset($product->cat_id)) { $product->cat_id = ''; } ?>
<?php if (!isset($product->dynamic_link)) { $product->dynamic_link = ''; } ?>
<?php if (!isset($product->original_price)) { $product->original_price = ''; } ?>
<?php if (!isset($is_discount)) { $is_discount = 0; } ?>
<?php if (!isset($product->unit_price)) { $product->unit_price = ''; } ?>
<?php if (!isset($discount) || !is_object($discount)) { $discount = (object) ['percent' => 0]; } ?>
	                  ...
	                  <div class="col-md-4" style="height:100">
	                    ...
	                    <div class="thumbnail">
	                      ...
	                      <p class="text-center">
	                        ...
	                        <?php if (isset($images[0]) && !empty($images[0]->img_id) && !empty($product->id)): ?>
								<a data-toggle="modal" data-target="#deletePhoto" class="delete-img" id="<?php echo $images[0]->img_id; ?>" image="<?php echo isset($img->img_path) ? $img->img_path : ''; ?>">
