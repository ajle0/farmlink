			...
			<?php foreach($users->result() as $user): ?>
				...
				<tr>
					...
					<?php if ( $this->ps_auth->has_access( DEL )): ?>
						...
						<td>
							<a href='#' class='btn-delete' data-toggle="modal" data-target="#myModal" id="<?php echo $user->user_id;?>">
...
function runAfterJQ() {
	...
	$(document).ready(function(){
		...
		$('.btn-delete').click(function(){
			var id = $(this).attr('id');
			var baseDeleteUrl = "<?php echo $module_site_url . '/delete/'; ?>";
			$('.btn-yes').attr('href', baseDeleteUrl + id);
		});

		// Optional: Clear the link after modal closes
		$('#myModal').on('hidden.bs.modal', function () {
			$('.btn-yes').attr('href', '<?php echo $module_site_url . "/delete/"; ?>');
		});
	});
}
