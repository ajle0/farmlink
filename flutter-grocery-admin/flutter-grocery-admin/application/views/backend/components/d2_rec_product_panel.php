    	...
    	<?php $count = $this->uri->segment(4) or $count = 0; ?>
            <?php if ( ! empty( $data )): ?>
              	<?php foreach($data as $d): ?>
                    ...
                    $img_path = $default_photo->img_path;
                    $thumbnail_path = FCPATH . 'uploads/thumbnail/' . $img_path;
                    $original_path = FCPATH . 'uploads/' . $img_path;
                    if ( !empty($img_path) && file_exists($thumbnail_path) ) {
                  ?>
				          	<img src="<?php echo img_url( 'thumbnail/' . $img_path ); ?>" alt="Product Image" class="img-size-50">
                  <?php } else if ( !empty($img_path) && file_exists($original_path) ) { ?>
                      <img src="<?php echo img_url( $img_path ); ?>" alt="Product Image" class="img-size-50">
                  <?php } else { ?>
