...
class PS_Delete {
	...
	*/
	function delete_transaction_trigger( $id )
	{
        ...
        if ( !$this->CI->Transactioncount->delete_by( $conds )) {
	...
	*/
	function delete_product_trigger( $product_id )
	{
		...
		if ( !$this->CI->Prd_attribute->delete_by( $conds )) {
	...
	/**
	 * Delete the best choice and image under the collection
	 *
	 * @param      <type>  $id     The identifier
	 */
	function delete_best_choice( $collection_id, $enable_trigger = false )
	{		
		if ( ! $this->CI->Best_choice->delete( $collection_id )) {
		// if there is an error in deleting category,
	...
	*/
	...
	{		
		if ( ! $this->CI->Prd_attribute->delete( $attribute_id )) {
	...
	{		
		...
		// if there is an error in deleting addon,
			
			return false;
		}

		// prepare condition
		$conds = array( 'img_type' => 'food-additional', 'img_parent_id' => $id );
	...
	/**
	 * Delete the shop schedule
	 *
	 * @param      <type>  $id     The identifier
	 */
	function delete_shop_schedule( $shop_id )
	{		
		$conds = array( 'shop_id' => $shop_id );
		if ( ! $this->CI->Schedule->delete_by( $conds )) {
		// if there is an error in deleting Product,
			return false;
		}
	}

	/** delete grocery branch */
	 ...
	 * @param      <type>  $id     The identifier
	*/
	function delete_grocery_branch( $grocery_id, $enable_trigger = false )
	{		
		if ( ! $this->CI->Grocery_branch->delete( $grocery_id )) {
		// if there is an error in deleting Product,
		...
		// prepare condition
		$conds = array( 'img_type' => 'restaurant-branch', 'img_parent_id' => $grocery_id );
	 ...
	 * @param      <type>  $id     The identifier
	*/
	function delete_schedule_order( $id ,$enable_trigger = false)
	{	

		if ( ! $this->CI->Schedule_header->delete( $id )) {
		// if there is an error in deleting Product,
		...
		if ( $enable_trigger ) {
		// if execute_trigger is enable, trigger to delete schedule order related data
			
			if ( !$this->delete_schedule_order_trigger( $id )) {
			// if error in deleting schedule order related data,
	...
	/**
	* Trigger to delete schedule order related data when schedule order is deleted
	* delete schedule order related data
	*/
	function delete_schedule_order_trigger( $id )
	{
		$conds = array( 'schedule_header_id' => $id );

        // delete schdeule order detail
		if ( !$this->CI->Schedule_detail->delete_by( $conds )) {

			return false;
		}

        // delete transaction shcedule
        if ( !$this->CI->Trans_schedule->delete_by( $conds )) {
	...
	/**
	 * Delete the Schedule Order Route
	 *
	 * @param      <type>  $id     The identifier
	 */
	function delete_schedule_order_route( $route_id )
	{		
		$conds = array( 'route_id' => $route_id );
		if ( ! $this->CI->Schedule_Order_Route->delete_by( $conds )) {
	...
	/**
	 * Delete the Order Route
	 *
	 * @param      <type>  $id     The identifier
	 */
	function delete_order_route( $route_id, $enable_trigger = false )
	{	
		if ( ! $this->CI->Order_route->delete( $route_id )) {
		// if there is an error in deleting route,
			
			return false;
		}
		
		if ( $enable_trigger ) {
			// if execute_trigger is enable, trigger to delete order route related data
			
			if ( !$this->delete_order_route_trigger( $route_id )) {
			// if error in deleting order route related data,
	...
	/**
	* Trigger to delete order route related data when order route is deleted
	* delete order route related data
	* if update qty, refresh original qty
	*/
	function delete_order_route_trigger( $id )
	{

		$conds = array( 'route_id' => $id );
		
		// if update qty, refresh original qty
		$sch_orders = $this->CI->Schedule_Order_Route->get_all_by($conds);
		foreach($sch_orders->result() as $sch_order){
			
			$sch_details = $this->CI->Schedule_detail->get_all_by(['schedule_header_id' => $sch_order->schedule_header_id]);
			
			foreach($sch_details->result() as $sch_detail){
				if($sch_detail->qty_modify != 0){
					$data_detail['qty_modify'] = 0;
					$data_detail['qty'] = $sch_detail->qty_modify;
					
					$this->CI->Schedule_detail->save($data_detail, $sch_detail->id);
	
					$sch_header = $this->CI->Schedule_header->get_one($sch_detail->schedule_header_id);
					$data_header['total_item_count'] = $sch_header->total_item_count - $sch_detail->qty + $sch_detail->qty_modify;
					$data_header['sub_total_amount'] = $sch_header->sub_total_amount - ($sch_detail->qty * $sch_detail->price) + ($sch_detail->qty_modify * $sch_detail->price);
					$data_header['total_item_amount'] = $data_header['sub_total_amount'] + ($sch_header->tax_amount + $sch_header->shipping_amount + ($sch_header->shipping_amount * $sch_header->shipping_tax_percent)); 
					$data_header['balance_amount'] = $data_header['sub_total_amount'] + ($sch_header->tax_amount + $sch_header->shipping_amount + ($sch_header->shipping_amount * $sch_header->shipping_tax_percent)); 
					
					$this->CI->Schedule_header->save($data_header,$sch_header->id);
        ...
        // delete schdeule order route detail
		if ( !$this->CI->Schedule_Order_Route->delete_by( $conds )) {
