...
class PS_Adapter {
	...
	function convert_product( &$obj , $need_return = false)
	{
		...
		} else if(isset($obj->login_user_id_post) && $obj->login_user_id_post != "") {
			$conds['product_id'] = $obj->id;
	...
	function convert_shop( &$obj )
	{
		...
		// Always set ismulti to '1' for multi-shop mode
		$obj->ismulti = '1';
    ...
    // convert schedule header

    /*
	 * Customize Schedule Header object
	 *
	 * @param      <type>  $obj    The object
	 */
	function convert_schedule_header( &$obj )
	{
		if(gettype($obj) == 'object'){
			$conds['schedule_header_id'] = $obj->id;
			$tmp_schedule_detail = $this->CI->Schedule_detail->get_all_by($conds)->result();

			$obj->schedule_detail = $tmp_schedule_detail;
			...
			foreach ($obj as $item) {
				...
				$conds['schedule_header_id'] = $item->id;
				$tmp_schedule_details = $this->CI->Schedule_detail->get_all_by($conds)->result();

				foreach($tmp_schedule_details as $detail){
					// print_r($detail);die;
					$item->schedule_detail[] = $this->convert_schedule_detail($tmp_schedule_details);
				}

				// shop object
				if (isset($item->shop_id)) {
					$tmp_shop = $this->CI->Shop->get_one($item->shop_id);
					$this->convert_shop($tmp_shop);
	 ...
	 * @param      <type>  $obj    The object
	 */
	function convert_schedule_detail( &$obj )
	{
	...
	/**
	 * Customize Order Route object
	 */
	function convert_order_route( &$obj ){
		// schedule object
		if ( isset( $obj->id )) {
			$schedules = $this->CI->Schedule_Order_Route->get_all_by( ['route_id' => $obj->id] )->result();
			
			foreach($schedules as $schedule){
				$sch_header_ids[] = $schedule->schedule_header_id;
				
			}

			$tmp_schedules = $this->CI->Schedule_header->get_all_by( ['sch_header_ids' => $sch_header_ids] )->result();
			$obj->schedule_orders = $tmp_schedules;
		}
	}
	/**
