...
class Backend_configs extends BE_Controller {
	...
	function save( $id = false ) {
		...
		// if 'search_in_shop' is checked,
		if ( $this->has_data( 'search_in_shop' )) {
			$data['search_in_shop'] = 1;
		} else {
			$data['search_in_shop'] = 0;
		}
		...
		// search_in_shop_limit
		if ( $this->has_data( 'search_in_shop_limit' ) 
			&& !empty( $this->get_data( 'search_in_shop_limit' ))) {
			$data['search_in_shop_limit'] = $this->get_data( 'search_in_shop_limit' );
		} else {
			$data['search_in_shop_limit'] = 0;
		}
