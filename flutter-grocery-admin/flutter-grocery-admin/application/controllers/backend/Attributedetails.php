...
class Attributedetails extends BE_Controller {
	...
	function index($header_id = 0,$product_id=0) {
		...
		//print_r($prd_attid);die;

		if ($url_id == "prd") {
			$this->session->set_userdata(array("product_id" => $product_id ));
			$this->session->set_userdata(array("header_id" => $header_id ));
		} else{
	...
	function search( $header_id = 0,$product_id=0 ) {
		...
		if($this->input->post('submit') != NULL ){
			...
			$header_id = $this->get_data( 'header_id' );
			$this->session->set_userdata(array("header_id" => $header_id ));
	...
	function add($header_id = 0,$product_id=0) {
		...
		$this->data['product_id'] = $this->Prd_attribute->get_one_by(array("id" =>  $header_id ))->product_id;
