 ...
 */
class Shopusers extends BE_Controller {
	...
	function index() {
		...
		// get rows count
		$this->data['rows_count'] = $this->User_shop->count_all($conds);

		// get users
		$this->data['users'] = $this->User_shop->get_shop_admin($conds, $this->pag['per_page'], $this->uri->segment( 4 ) );
	...
	function delete( $user_id ) {
		...
		if ( !$this->check_trans()) {
			...
			$this->set_flash_msg( 'success', get_msg( 'success_sys_user_delete' ));
		}
	...
	//print_r($permissions);die;
		if ($user['prdselect'] != "") {
			...
			if(count($user['prdselect']) == 1) {
				...
				if (empty($user['prdselect'][0])) {
					$select_data['shop_id'] = $this->get_data('shop_id');
				} else {
	...
	{		
		...
		// if the email is existing email for that user id,
			...
			return true;
		} else if ( $this->User->exists( array( 'user_email' => $_REQUEST['user_email'], 'status_not' => 0 ))) {
