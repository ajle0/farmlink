...
class Products extends BE_Controller {
		   ...
		   if ( $this->has_data( 'sub_cat_id' )) {
			...
			if ( $this->has_data( 'original_price' )) {
				$data['original_price'] = $this->get_data( 'original_price' );
				 ...
				 // if update price, change schedule product
				if($id){
					$sch_detail_count = $this->Schedule_detail->count_all_by(['product_id' => $id]);
					$sch_detail_price = array(); $sch_header_price = array(); $data_route_price = array();
					if($sch_detail_count != 0){
					 	$sch_details = $this->Schedule_detail->get_all_by(['product_id' => $id]);
						...
						foreach($sch_details->result() as $sch_detail){
							...
							if($data['unit_price'] != $data['original_price']){
								$sch_detail_price['discount_amount'] = $data['original_price'] - $data['unit_price'];
								$sch_detail_price['discount_value'] = $data['unit_price'];

								$sch_header_price['discount_amount'] = $sch_header->discount_amount - $sch_detail->discount_amount + $sch_detail_price['discount_amount'];
							}
							
							$sch_detail_price['original_price'] = $data['original_price'];
							$sch_detail_price['price'] = $data['unit_price'];

							$sch_header_price['sub_total_amount'] =  $sch_header->sub_total_amount - ($sch_detail->qty * $sch_detail->price) + ($sch_detail->qty * $data['unit_price']);
							$sch_header_price['total_item_amount'] = $sch_header_price['sub_total_amount'] + ($sch_header->tax_amount + $sch_header->shipping_amount + ($sch_header->shipping_amount * $sch_header->shipping_tax_percent));
							$sch_header_price['balance_amount'] = $sch_header_price['sub_total_amount'] + ($sch_header->tax_amount + $sch_header->shipping_amount + ($sch_header->shipping_amount * $sch_header->shipping_tax_percent));
							
							$data_route_price['total_amount'] = (float) $order_route->total_amount - (float)$sch_header->sub_total_amount + $sch_header_price['sub_total_amount'];
							
							$this->Schedule_detail->save($sch_detail_price, $sch_detail->id);
							$this->Schedule_header->save($sch_header_price, $sch_header->id);
	...
	*/
	...
	function delete( $id ) 
	{
		...
		// change amount in schedule order
		...
		if($sch_detail_count != 0){
			...
			foreach($sch_details->result() as $sch_detail){
				...
				$qty = $sch_detail->qty_modify == 0 ? $sch_detail->qty : $sch_detail->qty_modify;

				$sch_detail_prd['discount_amount'] = 0;
				$sch_detail_prd['discount_value'] = 0;
				$sch_detail_prd['discount_percent'] = 0;
				$sch_detail_prd['price'] = 0;
				$sch_detail_prd['original_price'] = 0;
				$sch_detail_prd['qty'] = $qty;
				$sch_detail_prd['qty_modify'] = 0;
				
				$sch_header_prd['discount_amount'] = $sch_header->discount_amount - $sch_detail->discount_amount;
				$sch_header_prd['sub_total_amount'] = $sch_header->sub_total_amount - ($sch_detail->qty * $sch_detail->price); 
				$sch_header_prd['balance_amount'] = $sch_header_prd['sub_total_amount'] + ($sch_header->tax_amount + $sch_header->shipping_amount + ($sch_header->shipping_amount * $sch_header->shipping_tax_percent));
				$sch_header_prd['total_item_amount'] = $sch_header_prd['sub_total_amount'] + ($sch_header->tax_amount + $sch_header->shipping_amount + ($sch_header->shipping_amount * $sch_header->shipping_tax_percent));
				$sch_header_prd['total_item_count'] = $sch_header->total_item_count - $qty;
				
				$data_route_price['total_amount'] = (float)$order_route->total_amount - (float)$sch_header->total_item_amount + $sch_header_prd['total_item_amount'];

				$this->Schedule_detail->save($sch_detail_prd, $sch_detail->id);
				$this->Schedule_header->save($sch_header_prd, $sch_header->id);
				$this->Order_route->save($data_route_price, $order_route->id);
	...
	function get_all_products_for_best_choice($collection_id = '000') 
	{
		...
		if ($collection_id=='000') {
 		...
 		// Datatables Variables
		  ...
		  if($product_collection) {
	          ...
	          $prd_dis = $this->Product->get_all_product_best_choice($conds);
