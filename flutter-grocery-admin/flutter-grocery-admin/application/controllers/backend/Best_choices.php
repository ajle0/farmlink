 ...
 */
class Best_choices extends BE_Controller
{
	/**
	...
	{
		parent::__construct( MODULE_CONTROL, 'BEST CHOICES' );
		///start allow module check by MN
		$selected_shop_id = $this->session->userdata('selected_shop_id');
		$shop_id = (is_array($selected_shop_id) && isset($selected_shop_id['shop_id'])) ? $selected_shop_id['shop_id'] : null;
	...
	function index() {
		...
		// get rows count
		$this->data['rows_count'] = $this->Best_choice->count_all_by( $conds );

		// get discounts
		$this->data['collections'] = $this->Best_choice->get_all_by( $conds , $this->pag['per_page'], $this->uri->segment( 4 ) );

		// load index logic
		parent::best_choice_index();
	}
	...
	*/	
	...
	if($prdcheck != "") {
			...
			// save collect
			if ( ! $this->Best_choice->save( $data, $id )) {
			...
			if ( ! $this->check_trans()) {
				...
				// if user id is not false, show success_add message
					...
					$this->set_flash_msg( 'success', get_msg( 'success_best_choice_edit' ));
				} else {
				// if user id is false, show success_edit message
					
					$this->set_flash_msg( 'success', get_msg( 'success_best_choice_add' ));
				}
			}
			...
			$this->set_flash_msg( 'error', get_msg( 'please_select_product' ) );
				if ( !$id ){
					redirect( site_url() . '/admin/best_choices/add');
				} else {
					redirect( site_url() . '/admin/best_choices/edit/' . $id);
				}
		}
	...
	*/
	...
	$this->data['prds_list'] = $this->Product->get_all_by( $conds, 40, $this->uri->segment( 5 ) );
		...
		// load user
	$this->data['collection'] = $this->Best_choice->get_one( $id );

	// call the parent edit logic
	parent::best_choice_edit( $id );
	}
	...
	function delete( $collection_id ) {
		...
		if ( !$this->ps_delete->delete_best_choice( $collection_id )) {
		...
		if ( !$this->check_trans()) {
			...
			$this->set_flash_msg( 'success', get_msg( 'success_best_choice_delete' ));
		}
	...
	{		
		$conds['name'] = $name;
     	...
     	// $conds['shop_id'] = $shop_id;
			...
			if ( strtolower( $this->Best_choice->get_one( $id )->name ) == strtolower( $name )) {
			// if the name is existing name for that user id,
				return true;
			} else if ( $this->Best_choice->exists( ($conds ))) {
	...
	function ajx_publish( $collection_id = 0 )
	{
		...
		// save data
		if ( $this->Best_choice->save( $collection_data, $collection_id )) {
			echo 'true';
		} else {
			echo 'false';
		}
