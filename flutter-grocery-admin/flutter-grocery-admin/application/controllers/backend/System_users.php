...
class System_users extends BE_Controller {
	...
	function save( $user_id = false ) {
		...
		// VALIDATION: ensure correct user_id is passed
		...
		if ($user['prdselect'][0] != "") {
			...
			if ($user_id == "") {
				$userInfo = $this->User->get_one_user_email($user_data)->result();	
				if (empty($userInfo) || empty($userInfo[0]->user_id)) {
					// No user found, generate a new user_id
					$user_data['user_id'] = $this->User->generate_key('USR');
				} else {
					$user_data['user_id'] = $userInfo[0]->user_id;
				}
				// save data with permission access
				if ( ! $this->User->save_system_user( $user_data, $permissions, $user_id )) {
					$this->set_flash_msg( 'error', get_msg( 'err_model' ));
				} 
			} else {
				// no need to give permission again
			...
			$userInfo = $this->User->get_one_user_email($user_data)->result();	
			$user_data['user_id'] = ( !$user_data['user_id'] ) ? $userInfo[0]->user_id : $user_data['user_id'];
			...
			if($user_id !=''){
				$user_is_sys_admin = $this->User->get_one($user_id);
				if($user_is_sys_admin){
					if ( ! $this->User->save( $user_data, $user_id )) {
						// if there is an error in inserting user data,	
						$this->set_flash_msg( 'error', get_msg( 'err_model' ));
					}
	...
	{		
		$conds = array('user_email' => $email);
		if ($user_id) {
			$conds['user_id !='] = $user_id; // Exclude current user
		}
		if ($this->User->exists($conds)) {
	...
	function delete( $user_id = null ) {
		if (empty($user_id)) {
			$this->set_flash_msg('error', 'Invalid user ID for deletion.');
			redirect($this->module_site_url());
			return;
		}
