...
class Attributes extends BE_Controller {
	...
	function index($product_id = 0) {
		...
		$prd_id = end($current_url);
		$url_id = substr($prd_id, 0,3);

		if ($url_id == "prd") {
			$this->session->set_userdata(array("product_id" => $product_id ));
		} else{
		...
		// get rows count
		$this->data['rows_count'] = $this->Prd_attribute->count_all_by( $conds );

		// get brands
		$this->data['attributes'] = $this->Prd_attribute->get_all_by( $conds , $this->pag['per_page'], $this->uri->segment( 4 ) );
	...
	*/
	...
	function edit( $id , $product_id = 0 ) {
		...
		// load user
		$this->data['attribute'] = $this->Prd_attribute->get_one( $id );
		$this->data['product_id'] = $product_id;
	...
	function delete( $attribute_id ) {
		$product_id = $this->Prd_attribute->get_one( $attribute_id )->product_id;
	...
	function save( $id = false , $product_id = 0 ) {
		...
		// save Attribute
		if ( ! $this->Prd_attribute->save( $data, $id )) {
	...
	{		
		 ...
		 $conds['product_id'] = $product_id;
			...
			if ( strtolower( $this->Prd_attribute->get_one( $id )->name ) == strtolower( $name )) {
			// if the name is existing name for that user id,
				return true;
			} 
			else if ( $this->Prd_attribute->exists( ($conds ))) {
