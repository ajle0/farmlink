...
class Register extends FE_Controller 
{
	...
	*/
	function save( $id = false ) {
		...
		if ( ! $this->User->save_user( $user_data,$user_id )) {
		...
		$user = $this->User->get_one_user_email($user_data)->result();
		
		$user_id = ( !$user_data['user_id'] ) ? $user[0]->user_id : $user_data['user_id'];
		...
		if ( $id ) {
			...
			if(!$this->ps_delete->delete_shop_tag( $id )) {
				//loop
				 //print_r($shop_data['prdselect']);die;
				for($i=0; $i<count($shop_data['prdselect']);$i++) {
					...
					if($shop_data['prdselect'][$i] != "") {
						...
						$select_data['added_user_id'] = $user_id;
		...
		// if paypal is enable,
			...
			// if there is an error in paypal transaction, redirect itself
				...
				$this->session->set_flashdata( 'error', get_msg('f_err_paypal_trans'));
				$this->db->trans_rollback();
	...
	*/
	...
	{
		$this->load_template( 'fb_login_success' );
	}
