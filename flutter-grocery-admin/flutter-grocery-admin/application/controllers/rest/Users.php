...
class Users extends API_Controller
{
	...
	function login_post()
	{
		...
		if (!$this->User->exists(array('user_email' => $this->post('user_email'), 'user_password' => $this->post('user_password')))) {
			$this->error_response(get_msg('err_user_not_exist'), 500);
		}
		...
		if ($code != "" && $code != " ") {
			$this->error_response(get_msg('need_to_verify'), 500);
		} else if ($is_banned == '1') {
			$this->error_response(get_msg('err_user_banned'), 500);
		} else if ($status == '0') {
			$this->error_response(get_msg('not_yet_activate'), 500);
		} else {
	...
	function deliboy_location_update_post()
	{
		...
		if ($this->User->get_one($user_id)->role_id == 5) {
			if (!$this->User->save($data, $user_id)) {
				...
				$this->success_response(get_msg('success_location_update'), 201);
			}
		} else {
	...
	*/
	function user_delete_post( ) {
        ...
        if ( $user_data->user_id == "" ) {
			...
			$sch_header_data = $this->Schedule_header->get_all_by($conds_user)->result();

			for ($i=0; $i <count($sch_header_data) ; $i++) { 
				$sch_header_update['user_id'] = "-1";
				$sch_header_update['schedule_status'] = "0";
				$sch_header_id = $sch_header_data[$i]->id;
				$this->Schedule_header->save($sch_header_update,$sch_header_id);
			}

			// delete schedule detail
			$sch_detail_data = $this->Schedule_detail->get_all_by($conds_added_user)->result();

			for ($i=0; $i <count($sch_detail_data) ; $i++) { 
				$sch_detail_update['added_user_id'] = "-1";
				$sch_detail_id = $sch_detail_data[$i]->id;
				$this->Schedule_detail->save($sch_detail_update,$sch_detail_id);
			}
	...
	*/
	function deliboy_delete_post( ) {
        ...
        if ( $user_data->user_id == "" ) {
			...
			$order_route_data = $this->Order_route->get_all_by($conds_deliboy)->result();

			for ($i=0; $i <count($order_route_data) ; $i++) { 
				$deli_update['delivery_boy_id'] = "-1";
				$order_route_id = $order_route_data[$i]->id;
				$this->Order_route->save($deli_update,$order_route_id);
			}
			...
			// delete schedule header
			$sch_data = $this->Schedule_header->get_all_by($conds_deliboy)->result();

			for ($i=0; $i <count($sch_data) ; $i++) { 
				$sch_update_deli['delivery_boy_id'] = "-1";
				$sch_id = $sch_data[$i]->id;
				$this->Schedule_header->save($sch_update_deli,$sch_id);
			}	
